...
AgentMeter 是原生 SwiftUI 应用，将 Codex、Claude、实验性的 TRAE SOLO CN 额度与手动登记的订阅和用量放在同一个面板中。内置豆包工作手动入口，也可以添加其他任意工具。支持的自动来源使用官方客户端已有的登录状态，记录保存在你的 Mac 上。
...
| TRAE SOLO CN | **实验性：** 使用本地客户端已有登录，读取个人账号（`marscode` scope）的积分包百分比 | 订阅信息；不支持的账号或数据不可用时可切换手动额度 |
| 豆包工作 | v1 不自动读取额度 | 内置订阅入口，最多两个手动额度窗口 |
| 任意其他工具 | v1 无自动额度接入 | 自定义订阅入口，最多两个手动额度窗口 |

手动订阅字段包括名称、套餐、续费或到期日期、月度金额、币种和备注。手动额度窗口记录已用百分比与可选重置时间，未知值可留空；保存额度变更时更新记录时间。这些读数始终标记为手动，不会自行刷新或重置。Codex、Claude 和 TRAE 均可主动切换为手动管理；自动读取失败不会悄悄更换数据来源。
...
TRAE 兼容实现针对本地 TRAE SOLO CN 0.1.63 凭据格式与内部用量接口，不支持企业、组织、`bytedance`、`saas` 或未知账号范围。它不使用 TRAE 企业 OpenAPI，也不会将积分包到期时间视为重置或续费时间。官方客户端变化时，此接入可能需要更新。
...
3. 使用自动额度时，在官方 Codex、Claude Code 或受支持的个人版 TRAE SOLO CN 客户端中登录，再刷新总览。
4. 编辑订阅日期和月度金额。豆包工作或手动管理模式可填写需要记录的额度；其他工具可添加自定义订阅。
...
TRAE 仅使用受支持个人账号已有的本地 TRAE SOLO CN 登录记录。豆包工作不读取登录信息；对应操作会打开官方应用，应用不可用时打开官网，便于你自行核对并记录用量。
...
Codex 请求经官方 CLI 执行，CLI 自行管理认证与网络行为。Claude 用量请求直接发送给 Anthropic；受支持个人账号的 TRAE 用量请求仅发送到固定的 `api.trae.cn` 接口。豆包工作等手动条目不读取凭据，也不发送额度请求。手动更新检查发送给 GitHub。凭据来源、存储内容与网络边界详见[隐私与数据来源说明](docs/PRIVACY.zh-CN.md)。
