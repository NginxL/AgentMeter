...
| TRAE SOLO CN 额度 | 从 `~/Library/Application Support/TRAE SOLO CN/User/globalStorage/storage.json` 读取 `iCubeAuthInfo://icube.cloudide` 登录记录，在内存中解码支持的加密格式 | 仅接受 `account.scope == "marscode"`，使用已有令牌以 `Cloud-IDE-JWT` 认证，向固定接口 `https://api.trae.cn/trae/api/v2/pay/ide_user_ent_usage` 发送 POST，请求字段为 `require_usage: true`、`req_source: 2`。不刷新令牌或写入凭据。 |
| 手动管理 | 用户填写的订阅与额度字段，包括豆包工作及其他服务商的手动回退 | 不读取手动条目的凭据，也不发送额度请求。 |
| 更新检查 | GET `https://api.github.com/repos/NginxL/AgentMeter/releases/latest` | 仅手动触发，不附带服务商凭据或订阅记录。发现新版本后，在浏览器中打开 GitHub 发布页。 |
| 打开官方面板或客户端 | 在浏览器中打开 `chatgpt.com/codex/settings/usage` 或 `claude.ai/settings/usage`；打开已安装的 TRAE SOLO CN 或豆包工作，找不到时回退到 `www.trae.cn` 或 `www.doubao.com/work` | 官方客户端或浏览器管理自身登录、Cookie 和网络行为；AgentMeter 不读取浏览器会话。 |

TRAE 适配器实验性兼容 TRAE SOLO CN 0.1.63 的本地格式与个人账号内部用量接口，不是公共企业 OpenAPI。未知或损坏的凭据格式会被拒绝；组织、企业、`bytedance`、`saas` 或未知账号范围会在发送网络请求前被拒绝。仅使用指定登录记录，不导入存储文件中的其他设置。Claude 和 TRAE 的 HTTP 会话禁用磁盘缓存与 Cookie，并拒绝重定向，避免将凭据转发到其他接口。
...
自动额度的重置时间描述服务商额度窗口，手动额度的重置时间由用户填写。TRAE 积分包到期时间不会映射为重置时间或订阅日期。v1 中所有订阅续费或到期日期都由用户手动填写，月度金额也相同。AgentMeter 不访问发票、付款方式或账单门户。
...
此命令清除 AgentMeter 的已保存记录和设置，不会退出 Codex、Claude Code、TRAE SOLO CN 或豆包工作登录，也不会删除它们的凭据。退出登录与删除凭据由对应官方客户端管理。
