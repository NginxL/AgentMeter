...
`scripts/test.sh` 执行 `swift run MeterChecks` 检查核心行为，执行 `swift run MeterProviderChecks` 检查 Codex/Claude 适配器，执行 `swift run MeterTraeChecks` 检查 TRAE 样本，并执行 `swift run AgentMeter --self-check` 检查应用模型集成。四者均使用离线样本；应用模型检查使用临时偏好设置域与模拟适配器。`scripts/build.sh` 创建应用包、用 AppKit 绘制图标、复制许可证说明、签名，并校验签名与 `Info.plist`。调试构建可使用 `CONFIGURATION=debug bash scripts/build.sh`。
...
| `Sources/MeterProviders` | 本地 Codex app-server、Claude 凭据与 HTTP 适配器、实验性的 TRAE SOLO CN 凭据与用量适配器 |
| `Sources/AgentMeter/AppModel.swift` | 刷新调度、本地存储、演示数据隔离、手动更新检查 |
| `Sources/AgentMeter/Views` | 原生 SwiftUI 总览、订阅管理与设置 |
| `Tests/MeterChecks` | 核心行为和响应处理的离线检查 |
| `Tests/ProviderChecks` | 使用临时模拟 CLI 与凭据样本的适配器离线检查 |
| `Tests/TraeChecks` | TRAE 合成凭据、不支持的账号范围、请求边界和积分包响应样本 |
| `Resources` 与 `scripts` | 应用元数据、原创图标生成、构建、测试和发布打包 |

额度适配器返回服务商读数，模型不据此推导账单日期。TRAE 仅支持个人 `marscode` 账号，其他范围在联网前拒绝，积分包到期时间不映射为额度重置。豆包工作和自定义条目采用手动管理；Codex、Claude 和 TRAE 可主动切换为手动额度。手动条目跳过服务商读取，保留自己的记录时间。
...
发布前还需检查两种界面语言、演示模式、两个手动额度窗口与记录时间、自动条目切换手动、额度不可用与过期状态、服务商登录失败、TRAE 不支持的账号范围，以及手动更新操作。README 截图必须来自真实应用界面，并明确标记示例数据。

可通过 `swift run AgentMeter --render-screenshots docs/images` 生成两种语言的原生界面演示截图。可选的 `swift run AgentMeter --probe codex`、`--probe claude` 或 `--probe trae` 会使用对应的真实本地登录状态进行只读额度检查；离线测试脚本与 CI 均不包含这一步。TRAE 组织账号被拒绝仅验证本地范围限制，不代表个人账号 HTTP 读取成功。
