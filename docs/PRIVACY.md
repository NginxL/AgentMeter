...
| TRAE SOLO CN quota | Read the `iCubeAuthInfo://icube.cloudide` login record from `~/Library/Application Support/TRAE SOLO CN/User/globalStorage/storage.json`; decode its supported encrypted format in memory | Only `account.scope == "marscode"` is accepted. Send its existing token as `Cloud-IDE-JWT` to the fixed POST endpoint `https://api.trae.cn/trae/api/v2/pay/ide_user_ent_usage`, with `require_usage: true` and `req_source: 2`. No token refresh or credential writes. |
| Manual tracking | User-entered subscription and quota fields; includes Doubao Work and manual fallback for other providers | No credentials are read and no quota request is sent for a manually tracked entry. |
...
| Open official dashboard or client | Open `chatgpt.com/codex/settings/usage` or `claude.ai/settings/usage` in your browser; open installed TRAE SOLO CN or Doubao Work, falling back to `www.trae.cn` or `www.doubao.com/work` | The official client or browser manages its own login, cookies, and network behavior. AgentMeter does not read browser sessions. |

The TRAE adapter is experimental compatibility with the TRAE SOLO CN 0.1.63 local format and an internal personal-account usage endpoint, not the public Enterprise OpenAPI. An unknown or damaged credential envelope is rejected. Organization, enterprise, `bytedance`, `saas`, and unknown account scopes are rejected before a network request. Only the selected login record is used; other settings in the storage file are not imported. Claude and TRAE HTTP sessions disable disk caching and cookies and reject redirects, so a credential is not forwarded to another endpoint.
...
Automatic quota reset timestamps describe the provider's quota window; manual quota reset times are user-entered. TRAE credit-pack expiration is not mapped to a reset timestamp or subscription date. Every subscription renewal or expiry date is a manual record in v1. Monthly amounts are also manual; AgentMeter does not access invoices, payment methods, or billing portals.
...
This clears AgentMeter's saved records and settings. It does not sign you out of Codex, Claude Code, TRAE SOLO CN, or Doubao Work or delete their credentials. Their official clients manage sign-out and credential removal.
